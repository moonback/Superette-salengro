    ...
    <section className="pb-20 lg:pb-0 h-full">
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-[1fr_360px] gap-6 lg:gap-8 items-start h-full">
        ...
        <div className="flex flex-col gap-6 lg:gap-8 w-full">
          ...
          <div className="flex flex-col gap-2 mx-2 lg:mx-0 lg:max-w-xl">
            <div className="flex p-1.5 bg-stone-100/80 backdrop-blur-md rounded-[1.25rem] border border-stone-200/60 shadow-inner w-full">
              <button
          ...
          {/* Massive Search Bar with Multiplier */}
            ...
            <div className="relative flex items-center">
              <div className="absolute left-4 z-10 flex items-center gap-2">
                {multiplier > 1 ? (
                  <button 
                    ...
                    className="flex items-center gap-1 bg-stone-900 text-white px-3 py-1.5 rounded-full font-black text-lg shadow-md hover:bg-stone-800 transition"
                  >
                    {multiplier}x
                    <X className="h-4 w-4 text-stone-400 ml-1" />
                  </button>
                ) : (
                  <Search className={`h-6 w-6 transition-colors ${mode === "out" ? "text-rose-400" : "text-emerald-400"}`} />
                )}
              </div>
              <input
                ref={searchInputRef}
                ...
                className={`w-full h-20 rounded-[2rem] border-2 bg-white/90 backdrop-blur-xl pl-[4.5rem] pr-6 text-lg font-bold text-stone-900 placeholder-stone-400 outline-none transition-all shadow-xl ${
                  mode === "out" 
                ...
                } ${multiplier > 1 ? (mode === "out" ? "border-rose-300 ring-4 ring-rose-500/10" : "border-emerald-300 ring-4 ring-emerald-500/10") : ""}`}
                onKeyDown={(e) => {
              ...
              {searchTerm && (
                <button
                  ...
                  className="absolute right-6 h-8 w-8 flex items-center justify-center rounded-full bg-stone-100 text-stone-500 hover:bg-stone-200 transition"
                >
                  <X className="h-4 w-4" />
                </button>
              )}
              ...
              {(searchTerm ? searchResults : inventory.slice(0, 12)).length === 0 ? (
                ...
                (searchTerm ? searchResults : inventory.slice(0, 12)).map((item) => {
                  ...
                  <button
                    key={item.barcode}
                    onClick={() => handleApplyProduct(item)}
                    className={`flex items-center gap-3 p-3 bg-white rounded-2xl border shadow-sm transition-all text-left group hover:-translate-y-0.5 hover:shadow-md ${
                      isLowStock ? "border-amber-200" : "border-stone-200/60"
                    } ${
                  ...
                  return (
                    <motion.div
                      ...
                      className={`bg-white rounded-2xl p-3.5 flex gap-3 shadow-sm border relative overflow-hidden ${
                        item.reverted ? "border-stone-200 grayscale" : isStockCritical ? "border-rose-300 bg-rose-50/30" : item.delta > 0 ? "border-emerald-100" : "border-rose-100"
                      }`}
                    >
                      {/* Left edge colored bar */}
                      <div className={`absolute left-0 top-0 bottom-0 w-1 ${
                        item.reverted ? "bg-stone-200" : isStockCritical ? "bg-rose-500" : item.delta > 0 ? "bg-emerald-400" : "bg-rose-400"
                      }`}></div>

                      <div className={`h-12 w-12 flex-shrink-0 rounded-xl flex items-center justify-center relative ${
