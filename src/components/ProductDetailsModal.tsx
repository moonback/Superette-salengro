  ...
  return (
    <div className="fixed inset-0 z-[70] bg-white sm:hidden">
      <div className="flex h-full flex-col">
        <header className="sticky top-0 z-10 border-b border-stone-200 bg-white px-4 pb-3 pt-[max(1rem,env(safe-area-inset-top))] ">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-4 py-4 pb-[calc(8rem+env(safe-area-inset-bottom))]">
          <section className="overflow-hidden rounded-2xl border border-stone-200 bg-stone-50">
            <div className="flex min-h-64 items-center justify-center bg-white p-6">
              {product.imageUrl ? (
                ...
                <div className="grid h-32 w-32 place-items-center rounded-2xl border border-dashed border-stone-300 bg-stone-50 text-stone-300">
                  <Package className="h-12 w-12" />
                </div>
              )}
            </div>

            <div className="space-y-3 border-t border-stone-200 bg-white p-6">
              <div className="flex flex-wrap gap-2">
                <span className="inline-flex items-center rounded-full bg-stone-50 px-4 py-1 text-xs font-bold text-slate-900">
                  {product.category || "Non classe"}
                </span>
                {product.brand && (
                  <span className="inline-flex items-center rounded-full bg-stone-100 px-4 py-1 text-xs font-semibold text-stone-600">
                    {product.brand}
                  </span>
                )}
              </div>
                ...
                <div className="flex items-end justify-between gap-3">
                  ...
                  {typeof product.lastMovement === "number" && (
                    <div className="rounded-2xl bg-white px-4 py-2 text-right shadow-sm">
                      ...
                      <p className={`mt-1 text-sm font-bold ${product.lastMovement >= 0 ? "text-emerald-500" : "text-rose-500"}`}>
            ...
            <div className="rounded-2xl border border-stone-200 bg-white p-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="rounded-2xl bg-stone-50 p-3">
                  ...
                  <p className={`mt-1 font-semibold ${margin === null ? "text-stone-500" : margin >= 0 ? "text-emerald-500" : "text-rose-500"}`}>
        ...
        <footer className="fixed inset-x-0 bottom-0 z-10 border-t border-stone-200 bg-white px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3  sm:hidden">
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={onEditStock}
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-slate-900 px-4 text-sm font-semibold text-white  shadow-slate-900/10 transition active:scale-[0.98]"
            >
